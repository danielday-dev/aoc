...
long long examineRace(long long, long long);
...
int main(){
    ...
    long long times[numInputs];
    long long distances[numInputs];
    long long possible_results[numInputs];
    long long results = 1;
...
*/
long long examineRace(long long maxTime, long long goalDistance){
    //find min time
    //find max time
    //return max - min
    ...
    long long possibleValues;
    long long minimum;
    bool minFound = false;
    for(long long speed = 0; speed <= maxTime; speed++){

        long long boatDistance = (maxTime - speed) * speed;
        if (speed % 100000 == 0){
            printf("%i\n", boatDistance);
        }
